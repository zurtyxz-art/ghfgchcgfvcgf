...
class Database:
    ...
    def create_tables(self):
        tables = ["users", "categories", "groups", "products", "orders", "pending_orders"]
        for table in tables:
            try:
...
async def username_handler(message: Message, state: FSMContext):
    ...
    await message.answer("✏️ Пожалуйста, введите комментарий к заказу:")
    await state.set_state(UserStates.waiting_for_comment)
...
if __name__ == "__main__":
    ...
    # УДАЛЕН БЛОК, КОТОРЫЙ УДАЛЯЛ БАЗУ ДАННЫХ ПРИ ПЕРЕЗАПУСКЕ
